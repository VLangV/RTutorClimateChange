...
sc:
    - dday0C consists of the time where the temperature was higher than 0°C and therefore also includes the time were 1°C and more was measured.  Whereas dday1C only contains the daily percentage of time where the temperature was above 1°C.*
    - dday1C, since 1°C is a higher temperature than 0°C.
...
The two sides of the `case_when` functions are separated by the operator ~
- Left side: If condition 
- Right side: Formula which is executed when the if condition is true
Additional if-else statements can be added, separated by a comma.
...
$$RSS = \sum_{i=1}^{T} (y_{i} - x_{i}^T\hat\beta)^2 = (y-X\hat\beta)^T(y-X\hat\beta) = (y - \hat y)^2 = \hat\epsilon^2 \tag{2}$$
An OLS estimate $\hat\beta$ minimizes the sum of squared residuals $\hat\epsilon^2$ by deriving the equation for $RSS$ to $\hat\beta$:

$$\frac{\partial RSS}{\partial\hat\beta} = 0 \tag{3}$$
By solving the equation $\tag{3}$ for $\hat\beta$ we receive the following equation to calculate the estimates $\hat\beta$:

$$\hat\beta = (X^TX)^{-1} X^Ty \tag{4}$$
(source: Hastie et al.(2009), p.11-12)
#>
...
If this assumption holds we have exclusively exogenous explanatory variables in the regression model. This means that none of the explanatory variables $x_{k}$ are correlated with the error term $\epsilon$:
...
#< info "Summary Regression Output"
  ...
  * **Residuals** are the differences between the predicted values and the actual values $\hat u = y - \hat y$. In the summary output the distribution of the residuals is shown by the minimal, maximal and median value and the lower and upper quartile.
  * **Coefficients** 
    * **Estimates** is the column, where for each explanatory variable and the intercept an estimate $\hat \beta_{0}...\hat\beta_{n}$ is given which is used to predict the dependent variable of the regression.
    * **Std.Error** indicates the average value in which the estimator deviates from the actual value.
    * **t value** is the estimate $\hat \beta_{n}$ divided by the corresponding Standard Error.
    * **Pr(>|t|)** represents the p-value which shows at which **significance level** the null hypothesis can be rejected. The null hypothesis $H_{0}$ implies that the relationship between y and x was due to chance. 
  * **Residual standard error** is the average error of the Residuals. 
  * **degrees of freedom** shows the number of observations used for the regression minus the number of estimated coefficients.
  * **R-squared** is the percentage of variance of the dependent variable which can be explained by the explanatory variables.
  * **adjusted R-squared** is a measure for goodness-of-fit which controls by multiple regression for overfitting when adding explanatory variables. 
  * **F-statistics** is a general indicator if the relation between the dependent and all independent variables in the model is by chance, indicated by a p-value for the whole model.
...
question: Can we generally assume that increasing temperatures have a negative impact on corn ?
sc:
    - yes, rising temperatures have a negative impact on corn
    - no, only above a certain threshold temperature has a negative impact on corn growth*
...
question: Would you assume endogenous or exogenous variables in this model?
sc:
    - endogen*
    - exogen 
...
question: Which variable should we not forget to include in the model, otherwise there could be a bias due to omitted variables? 
sc:
    - a variable that is correlated with one (or more) of the explanatory variables. 
    - a variable that has an impact on corn production. 
    - a variable that is correlated with one (or more) explanatory variables and has an impact on corn production.*
...
sc:
    - No adaptation has been made*
    - Full adaptation has taken place 
...
sc:
    - No in this case, the negative impact of heat has increased in the long term
    - Yes, because then the negative influence of heat was reduced in the long term*
...
sc:
    - gdd_lo, the interval for growing degree days between 0°C and 29°C.
    - gdd_hi, the interval for growing degree days above 29°C.*
    - prec_lo, the total precipitation per year equal or lower than 42 cm.
    - prec_hi, the total precipitation per year higher than 42 cm. 
...
question: How many observation values n will each Panel bootstrap data set $\mathcal{X}^{*}$ have?
sc:
    - n = 31.724*
    - n = 1000
...
question: How many bootstrap replications of $\hat\beta_{2}^{P}$ are calculated?
sc:
    - n = 40762
    - n = 1000*
...
-	the median, drawn by a line inside the box
- the interquartile range $IQR=Q3-Q1=Q_{p=0.75}-Q_{p=0.25}$, represented as a box
     - lower quartil $Q_{p=0.25}$ 25% of the values are smaller or equal than the lower quartil
     - upper quartil $Q_{p=0.75}$ 75% of the values are smaller or equal than the upper quartil
-	the upper and lower whiskers which are 1.5 times the length of the IQR, shown as lines right and left to the box
- the outliers which are values outside of the whiskers, shown as dots
